...
public class HibernateDataAccess {
	...
	public Ride createRide(String from, String to, Date date, int nPlaces, /* float price */ List<Float> price,
		...
		try {
			...
			if (driver.doesRideExists(ibilbide, date)) {
				...
				db.close();
				System.out.println("ride alreadyExists");
	...
	/**
	 * Metodo honek bueltatzen ditu bi tokietatik pasatzen diren eta eselekuak libre
	 * dituzten bidaiak
	 * 
	...
	public List<RideContainer> getEginRidesOfDriver(Driver d) {
		 ...
		 * db.find(Driver.class, d.getUser()); List<Ride> rideList =
		 * driver.getEgitenRidesOfDriver(); List<RideContainer> emaitza = new
		 * ArrayList<RideContainer>(); for (Ride ride : rideList) { emaitza.add(new
		 * RideContainer(ride)); } return emaitza;
		 */
		return null;
	}
	...
	public List<RideContainer> getAllRidesOfDriver(Driver driver) {
		/*
		 * EntityManager db = JPAUtil.getEntityManager(); Driver d =
		 * db.find(Driver.class, driver.getUser()); List<Ride>rides=d.getRides();
		 * List<RideContainer> rideContainerList= new LinkedList<RideContainer>();
		 * for(Ride ride: rides) { rideContainerList.add(new RideContainer(ride)); }
	...
	public float getMoney(Profile profile) {
		...
		Profile p;
		try {
			p = db.find(Profile.class, profile.getUser());
		} catch (Exception e) {
			p = null;
		} finally {
			db.close();
	...
	public List<Kotxe> getKotxeGuztiak(Driver driver) {
		EntityManager db = JPAUtil.getEntityManager();
		List<Kotxe> kotxeGuztiak = new ArrayList<Kotxe>();
		try {
			db.getTransaction().begin();
			Driver d = db.find(Driver.class, driver.getUser());
			d.getKotxeGuztiak().size(); /// BERDINA EGITEN DUTE, GAUZA DA NIRE CONSOLE EZ BETETZEKO BEHAR EZ DUTEN
			/// INFOAZ
			// System.out.println(d.getKotxeGuztiak());
	...
	// ALDATU2
	public boolean sartuDirua(float dirua, Profile user) {
		EntityManager db = JPAUtil.getEntityManager();
	...
	// ALDATU2
	public boolean kenduDirua(float dirua, Profile user) {
		EntityManager db = JPAUtil.getEntityManager();
		boolean emaitza = false;
		try {
			db.getTransaction().begin();
	...
	public boolean onartuEdoDeuseztatuErreserba(RideRequest request, boolean onartuta) {
		...
		try {
			...
			RideRequest r = db.find(RideRequest.class, request.getId());
			Ride ride = db.find(Ride.class, r.getRide().getRideNumber());
	...
	public void erreklamazioaProzesatu(Erreklamazioa erreklamazioa, float kantitatea, boolean onartuta) {
		 ...
		 * e.setEgoera(EgoeraErreklamazioa.BUKATUA); Profile nork =
		 * db.find(Profile.class, e.getNork().getUser()); Profile nori =
		 * db.find(Profile.class, e.getNori().getUser()); Profile admin = e.getAdmin();
		 * RideRequest request = e.getErreserba(); Ride r = request.getRide(); if
		 * (onartuta) { erreklamazioaOnartu(nork, nori, admin, kantitatea,
		 * erreklamazioa); } else { erreklamazioaDeuseztatu(nork, admin, kantitatea,
		 * erreklamazioa); } e.setWhenDecided(new Date()); db.getTransaction().commit();
		 */
	}
	...
	public RideRequest erreserbatu(RideRequest request) {
		...
		try {
			...
			if (!eserlekuKopEgokia(rd, request)) {
				System.out.println("Eserleku kopuru desegokia");
				db.getTransaction().rollback();
				rq = null;
			} else {
	...
	public List<Erreklamazioa> getErreklamazioak(Profile p) {
		/*
		 * EntityManager db = JPAUtil.getEntityManager(); db.getTransaction().begin();
		 * Profile profile = db.find(Profile.class, p.getUser()); List<Erreklamazioa>
		 * erreklamazioak = profile.getErreklamazioak(); db.getTransaction().commit();
		 * if (erreklamazioak != null) return erreklamazioak; else return new
		 * ArrayList<Erreklamazioa>();
		 */
		return null;
	}
	...
	public void gehituErreklamazioa(Erreklamazioa erreklam) {
		/*
		 * EntityManager db = JPAUtil.getEntityManager(); Profile nork=
		 * erreklam.getNork(); RideRequest r= erreklam.getErreserba();
		 * db.getTransaction().begin(); Profile profile = db.find(Profile.class,
		 * nork.getUser()); RideRequest request = db.find(RideRequest.class, r.getId());
		 * if (profile instanceof Traveller) { request.setErreklamatuaDriver(true); }
		 * else { request.setErreklamatuaTraveller(true); }
		 * profile.gehituErreklamazioa(erreklam.getNori(), erreklam.getDeskripzioa(),
		 * erreklam.getPrezioa(), request); db.getTransaction().commit();
		 */
	...
	public Erreklamazioa takeNewErreklamazioa(Admin a) {
		 ...
		 * EntityManager db = JPAUtil.getEntityManager();
		 * 
		 * TypedQuery<Erreklamazioa> query =
		 * db.createQuery("SELECT e FROM Erreklamazioa e WHERE e.egoera=?1 ",
		 * Erreklamazioa.class);
		 * 
		 * 
		 * 
		 ...
		 * 
		 * List<Erreklamazioa> erreklamazioak = query.getResultList(); if
		 * (!erreklamazioak.isEmpty()) { db.getTransaction().begin(); Erreklamazioa
		 * erreklamazioa = erreklamazioak.get(0); Admin admin = db.find(Admin.class,
		 * a.getUser());
		 * 
		 * erreklamazioa.setEgoera(EgoeraErreklamazioa.PROZESUAN);
		 * 
		 * 
		 * erreklamazioa.setAdmin(admin); admin.addErreklamazioa(erreklamazioa);
		 * admin.gehituMezuaTransaction(8, 0, erreklamazioa.getErreserba());
		 * db.getTransaction().commit();
		 * 
		 * return erreklamazioa; } else
		 */
		return null;
	}
	...
	public void mezuaIrakurrita(Mezua alerta) {
		/*
		 * EntityManager db = JPAUtil.getEntityManager(); db.getTransaction().begin();
		 * Mezua a = db.find(Mezua.class, alerta.getId()); a.setIrakurritaTrue();
		 * db.getTransaction().commit();
		 */
	}
	...
	public boolean isErreklamatua(RideRequest request, boolean gidari) {
		 ...
		 * r.isErreklamatuaTraveller(); } else { return r.isErreklamatuaDriver();
		 * 
		 * }
		 */
		return false;
	...
	public List<Erreklamazioa> lortuErreklamazioakProzesuan(Profile a) {
		...
		TypedQuery<Erreklamazioa> query = db
				.createQuery("SELECT e FROM Erreklamazioa e WHERE  e.admin=?2 AND e.egoera=?3", Erreklamazioa.class);
	...
	private void erreklamazioaOnartu(Profile nork, Profile nori, Profile admin, float kantitatea, Erreklamazioa e) {
		/*
		 * RideRequest request = e.getErreserba(); Ride r = request.getRide();
		 * 
		 * nork.gehituDirua(kantitatea); nori.kenduDirua(kantitatea);
		 * nork.addErreklamazioMezu(0, r, e); nork.gehituMezuaTransaction(9, kantitatea,
		 * request); nori.gehituMezuaTransaction(11, kantitatea, request);
	...
	private void erreklamazioaDeuseztatu(Profile nork, Profile admin, float kantitatea, Erreklamazioa e) {
		 ...
		 * RideRequest request = e.getErreserba(); Ride r = request.getRide();
		 * 
		 * nork.addErreklamazioMezu(1, r, e); admin.addErreklamazioMezu(3, r, e);
		 */
	}
